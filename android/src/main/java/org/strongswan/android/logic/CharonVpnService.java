...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if (intent != null)
		{
			...
			//	profile = mDataSource.getVpnProfile(uuid);
				profile = VpnManagerImplNew.getInstance().getVpnProfile();
			}
	...
	private Notification buildNotification(boolean publicVersion)
	{
		...
		if (error != ErrorState.NO_ERROR)
		{
			...
			if (!publicVersion && profile != null)
			{
				...
				Intent intent = new Intent(getApplicationContext(), VpnActivityWrapper.class);
				intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				intent.setAction(VpnActivityWrapper.START_PROFILE);
				intent.putExtra(VpnActivityWrapper.EXTRA_VPN_PROFILE_ID, profile.getUUID().toString());
				...
				builder.addAction(R.drawable.ic_notification_connecting, getString(R.string.retry), pendingIntent);
				add_action = true;
			}
		}
		else
		...
		if (!publicVersion)
		{
			...
			{
				Intent intent = new Intent(getApplicationContext(), VpnActivityWrapper.class);
				intent.setAction(VpnActivityWrapper.DISCONNECT);
				...
				builder.addAction(R.drawable.ic_notification_disconnect, getString(R.string.disconnect), pendingIntent);
			}
			if (error == ErrorState.NO_ERROR)
			{
	...
	public class BuilderAdapter
	{
		...
		private VpnService.Builder createBuilder(String name)
		{
			...
			Intent intent = new Intent(context, VpnActivityWrapper.class);
			PendingIntent pendingIntent = null;
